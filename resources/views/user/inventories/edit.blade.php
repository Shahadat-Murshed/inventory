...
@section('content')
    ...
    <h2 class="mt-5">Editing {{ $inventory->name }}</h2>
    <div class="d-flex justify-content-center mt-5">
        <div class="col-md-8 card p-5">
            <form class="col-md-12" method="POST" action="{{ route('user.inventories.update', $inventory->id) }}">
                @method('PUT')
                @csrf
                ...
                <div class="form-group">
                    ...
                    <textarea class="form-control" id="description" rows="3" name="description" value="{{ old('description') }}"
                        placeholder="Write a short Description (optional).">{{ $inventory->description }}</textarea>
                </div>
